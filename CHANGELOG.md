...
A security remediation release. It closes every actionable finding of the 2026-09-08
offensive audit, each of which lived **between layers** rather than inside a control: an
identifier normalised in one place and trusted in another, an identity derived twice by
two implementations, a lifecycle whose state half and disk half were each correct alone
and connected to nothing.

Three changes are incompatible with 1.x. **Before upgrading:**

1. If you relied on `require_auth = true`, declare your own guard instead. One entry
   covers all five endpoints: `'routes' => ['middleware' => ['api', 'auth:sanctum']]`.
2. If you create sessions programmatically through `StateRepositoryInterface`, pass a
   `SessionOwner`. A session with no owner is now reachable by nobody.
3. If you bind your own `ResolvesCallerIdentity`, make sure it returns a scheme-qualified
   identity such as `tenant:7` or `api-key:abc123`.

Scheduling `stateful-chunking:clear-stale` also moves from suggested to **required**: it
was a no-op before this release, so abandoned uploads were retained regardless.
...
<!--
  Note: this release's entries cover the work that followed the 1.0.0 tag and were
  written as it happened. Changes made between the tag and the start of that record
  were not tracked in a changelog; backfill them here if they surface.
-->
...
[Unreleased]: https://github.com/JuanGuerreroDev/stateful-chunking-package/compare/v2.0.0...HEAD
[2.0.0]: https://github.com/JuanGuerreroDev/stateful-chunking-package/compare/v1.0.0...v2.0.0
